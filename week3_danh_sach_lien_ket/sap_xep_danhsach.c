...
void arrange()
{
    ...
    while(p1->next != NULL)
    {
        ...
        while(p2 != NULL)
        {
            if (p1->infor.diem < p2->infor.diem)
            {
                swap(&p1->infor,&p2->infor);
            }
            p2 = p2->next;
...
void display()
{
    ...
    {
        printf("\n");
        printf("%-30s%-15s%-30s%-5f",p->infor.name,p->infor.phone,p->infor.email,p->infor.diem);
    }
}
void Input()
{
