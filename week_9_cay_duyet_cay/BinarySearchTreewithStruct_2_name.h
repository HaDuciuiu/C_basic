...
{ 
	...
	else if (strcmp(Root->key.name,x)==0) /* found x */ 
		return Root; 
...
// ham 4 . In ra file theo thu tu name tang dan - thuc chat la duyet cay Inorder
void InOrder_file(TreeType root,FILE *fout)
{
    if(root!=NULL)
    {
        InOrder_file(root->left,fout);
        fprintf(fout,"%s\n%s\n%s\n",root->key.name,root->key.phone,root->key.email);
        InOrder_file(root->right,fout);
    }
}
// ham 5 . Tim kiem theo chieu rong - Breath First Search
void breadth_first(TreeType node)
{
	printf("Mo dau ham\n");
   Queue Q; // queue of pointers
   MakeNullQueue(&Q);	
   if (node!=NULL)
   {
       EnQueue(node,&Q);
	   ...
	   {
		  node = DeQueue(&Q);
          printf("%s\n%s\n%s",node->key.name,node->key.phone,node->key.email);
          if (node->left != NULL)
            EnQueue(node->left,&Q);
          if (node->right!= NULL)
